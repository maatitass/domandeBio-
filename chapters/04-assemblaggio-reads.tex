\chapter{Definizione di grafo di De Brujin e grafo di overlap e assemblaggio delle read}
...
tre fasi:
\begin{itemize}
    \item Calcolo degli \textbf{overlap} tra i read
...
A ogni arco $(r_i,r_j)$, inoltre, si associa inoltre un'etichetta:
...
Quindi l'assemblaggio non consiste nel concatenare interamente tutti i read, 
...
Il secondo metodo per il contig assembly è basato sui \textbf{grafo di De Bruijn (dBG)}, che invece di lavorare direttamente sulle read,
...
è il multiset di tutti i k-mers presenti in essa. Si parla di multiset perchè uno stesso k-mer
può comparire più volte, e ogni occorrenza è considerata distinta.

IL goal di metodo è assemblare una sequenza di DNA attraverso la
...
è un grafo orientato $G = (V, A)$, dove $V = \Sigma^k$ cioè i vertici sono tutte le possibile stringhe di lunghezza $k$ costruite sull'alfabeto $\Sigma$.
...
\]
Ogni arco $(v_1, v_2)$ rappresenta quindi la possibilità di concatenare $v_1$ e $v_2$ sovrapponendo i loro $k-1$.
...
\[
    V = \{\text{k-mer distinti nelo spettro di ordine k delle read}\}
\]
Quinfi i nodi nel grafo sono i k-mers estratti dalle read.
Esiste un arco orientato $(v_1, v_2)\in A$ se e solo il suffisso di $v_1$ di lunghezza $k-1$ è uguale al prefisso di $v_2$ di lunghezza $k-1$:
\[
    (v_1, v_2) \in A \iff suff_{k-1}(v_1) = pref_{k-1}(v_2)
\]
...
\]
 \begin{figure}[H]
    ...
    \caption{grafo di De Bruijn node-centric ordine 5}
...
un arco dal prefisso di lunghezza $k-1$ alsuo suffisso di lunghezza $k-1$.
...
\]
 ...
 \begin{figure}[H]
    ...
    \caption{grafo di De Bruijn arc-centric ordine 5}
